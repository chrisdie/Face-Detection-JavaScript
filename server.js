...
let nextVisitorNumber = 0;
let data = [] 
let visitorexpressions = {}
...
function onNewWebsocketConnection(socket) {
  ...
  socket.on("disconnect", () => {
      console.info(`Socket ${socket.id} has disconnected. There are now ${nextVisitorNumber} visitors...`);
      socket.emit("visitorLeft", nextVisitorNumber);
  });
...
function startServer() {
  ...
  app.post("/newdata", (req, res) => {
    data = req.body
    visitorexpressions[data.visitorid] = data.expression
    console.log("**** newdata", data)
    io.emit("visitorexpressions", visitorexpressions);
    return res.send(200);
  });
